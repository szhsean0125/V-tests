...
def time_of_day(d,b):
    """Convert a day and AM/PM Boolean to a me unit.
    """
    if d == 0:
...
def day_of_time(n):
    """Calculate which day is associated with a time unit.
    """
    return ((n + 1) // 2)

# This function will convert a "time" to an AM/PM Boolean
def period_of_time(n):
    """Determine if a time is AM or PM (day 0 is "AM" by convention)
    """
    if n == 0:
        return True
    return ((n + 1) % 2 == 0)
...
def is_initial(n):
    """Check if a time unit is day 0.
    """
    return n == 0

# This function turns a "time" into a day/am-pm string
def str_time(t):
    """Convert a time unit into a string.
    """
    if is_initial(t):
        return "0"
